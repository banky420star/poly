...
fn print_dashboard(
    ...
    // Fade strategy: invert direction labels
    // BTC ↑ → MMs need juice → push down → BUY NO
    // BTC ↓ → MMs need juice → push up → BUY YES
    let (direction, dir_color) = if momentum_signal > 0.55 {
        ("BTC ↑  →  BUY NO (fade)", GREEN)
    } else if momentum_signal < 0.45 {
        ("BTC ↓  →  BUY YES (fade)", RED)
    } else {
        ("BTC →  →  HOLD", YELLOW)
    };
...
async fn run_bot(config_path: &str, paper: bool, live: bool) -> Result<()> {
    ...
    let arb_engine = arb::ArbEngine::new(
        rust_decimal_macros::dec!(0.980), // max combined ask: $0.980 (2% gross → 0.4% net after 1.6% fees)
        rust_decimal_macros::dec!(10),    // max $10 per pair
    );
    ...
    loop {
        ...
        let mut ai_signal: Option<crate::models::AiSignal> = None;
        let mut shadow_mode = false;
        if let Some(ai_cfg) = &cfg.ai {
            if ai_cfg.enabled {
                shadow_mode = ai_cfg.shadow_mode;
                if let Ok(raw) = std::fs::read_to_string(&ai_cfg.signal_file) {
                    if let Ok(sig) = serde_json::from_str::<crate::models::AiSignal>(&raw) {
                        if let Ok(ts) = chrono::DateTime::parse_from_rfc3339(&sig.timestamp) {
                            let age_ms = chrono::Utc::now().signed_duration_since(ts.with_timezone(&chrono::Utc)).num_milliseconds();
                            if age_ms >= 0 && age_ms <= ai_cfg.max_signal_age_ms as i64 {
                                tracing::info!("Loaded valid AI signal for token={}", sig.token_id);
                                ai_signal = Some(sig);
                            } else {
                                tracing::warn!("AI signal stale (age={}ms > max={}ms)", age_ms, ai_cfg.max_signal_age_ms);
                            }
                        }
                    } else {
                        tracing::warn!("Failed to parse AI signal JSON");
                    }
                }
            }
        }
        ...
        // ── Update equity and compounding BEFORE strategy generates quotes ──
        // Live mode: refresh real balance for equity calculation.
        // Compounding scales risk limits based on current equity — this must
        // happen before the strategy runs so limits match our bankroll.
        ...
        let equity = if live {
            ...
            let position_value: rust_decimal::Decimal = ledger.positions.values()
                .map(|p| p.cost_usdc)
                .sum();
            ledger.cash_usdc + position_value
        };

        if let Some(ref compound_cfg) = cfg.compound {
            if compound_cfg.enabled && equity > rust_decimal::Decimal::ZERO {
                risk.update_for_equity(equity, Some(compound_cfg));
                let eq_f = equity.to_f64().unwrap_or(0.0);
                let base_bet = rust_decimal::Decimal::from_f64(eq_f * compound_cfg.base_bet_pct)
                    .unwrap_or(rust_decimal_macros::dec!(5));
                // Floor at $5 to meet CLOB minimum notional requirement
                ledger.default_order_size_usdc = base_bet.max(rust_decimal_macros::dec!(5));
                // Update executor's max order cap so it doesn't clamp bets.
                // Floor at CLOB minimum $5 so small accounts can still place orders.
                if let Some(ref mut exec) = live_executor {
                    exec.max_order_usdc = rust_decimal::Decimal::from_f64(eq_f * compound_cfg.max_order_pct)
                        .unwrap_or(rust_decimal_macros::dec!(5))
                        .max(rust_decimal_macros::dec!(5));
                }
            }
        }

        // Cancel stale orders every N loops to prevent accumulation while
        // allowing maker quotes time to rest on the book between cancellations.
        // Default: cancel every 5 loops (15s at default 3s interval).
        const CANCEL_INTERVAL_LOOPS: u64 = 5;
        if let Some(exec) = &live_executor {
            if iteration % CANCEL_INTERVAL_LOOPS == 0 {
        ...
        for market in selected.iter() {
            ...
            if let Some(ref d) = depth {
                let time_left = strategy::time_left_seconds(market).unwrap_or(0);
                log_training_tick(
                    market,
                    d,
                    ...
                    feed.btc_return_30s(),
                    momentum_signal,
                    time_left.try_into().unwrap_or(0),
                );
            }
            ...
            // Zero depth means no liquidity at all — skip those too.
            ...
            // ── Pair-cost arbitrage: guaranteed profit first ──
            if let Some(ref books) = book_data {
                if let (Some(yes_token), Some(no_token)) = (market.yes_token_id(), market.no_token_id()) {
                    if let (Some(yes_book), Some(no_book)) = (books.get(&yes_token), books.get(&no_token)) {
                        if let Some((arb_yes, arb_no)) = arb_engine.check_arb(market, yes_book, no_book, &ledger) {
                            if arb_yes.size_usdc > rust_decimal_macros::dec!(0) {
                                // Execute sequentially: buy the thinner side first,
                                // wait for fill, then buy the other side. This guarantees
                                // both sides fill before we count the arb as complete.
                                if let Some(exec) = &mut live_executor {
                                    // Thinner depth first (harder to fill)
                                    let yes_depth = yes_book.spread_info().depth_bid_usdc;
                                    let no_depth = no_book.spread_info().depth_bid_usdc;
                                    let (first, second) = if yes_depth <= no_depth {
                                        (arb_yes.clone(), arb_no.clone())
                                    } else {
                                        (arb_no.clone(), arb_yes.clone())
                                    };

                                    match exec.execute_order(&first).await {
                                        Ok(result) if result.filled => {
                                            tracing::info!(
                                                size = %first.size_usdc,
                                                side = %first.outcome,
                                                "ARB: first leg filled — now buying second leg"
                                            );
                                            match exec.execute_order(&second).await {
                                                Ok(result2) if result2.filled => {
                                                    let yes_sz = arb_yes.size_usdc;
                                                    let no_sz = arb_no.size_usdc;
                                                    tracing::info!(
                                                        yes = %yes_sz,
                                                        no = %no_sz,
                                                        "ARB: BOTH legs filled — risk-free profit locked"
                                                    );
                                                    let _ = ledger.submit_quote_intent(&cfg.paper, arb_yes)?;
                                                    let _ = ledger.submit_quote_intent(&cfg.paper, arb_no)?;
                                                    buys += 2;
                                                }
                                                Ok(_) => {
                                                    tracing::warn!("ARB: second leg resting — cancel and skip");
                                                    // Cancel unfilled second leg
                                                    let _ = exec.cancel_all_orders().await;
                                                }
                                                Err(e) => {
                                                    tracing::warn!("ARB: second leg failed: {}", e);
                                                }
                                            }
                                        }
                                        Ok(_) => {
                                            tracing::warn!("ARB: first leg not filled — arb aborted");
                                        }
                                        Err(e) => {
                                            tracing::warn!("ARB execution failed: {}", e);
                                        }
                                    }
                                } else {
                                    // Paper mode
            ...
            let quotes = strategy.build_quotes(
                market, &ledger, momentum_signal, time_left, ws_books, depth.as_ref(), ai_signal.as_ref(), shadow_mode
            )?;
...
/// Log AI training ticks to training_ticks.jsonl
pub fn log_training_tick(
    market: &crate::models::GammaMarket,
    d: &DepthData,
    btc_price: f64,
    btc_return_5s: f64,
    btc_return_30s: f64,
    ...
    let now = chrono::Utc::now().to_rfc3339();
    let slug = &market.id;
    let token_id_yes = market.yes_token_id().unwrap_or_default();
    let token_id_no = market.no_token_id().unwrap_or_default();
    
    // Convert rust_decimal::Decimal to f64
    use rust_decimal::prelude::ToPrimitive;
    let yes_bid = d.yes_best_bid.and_then(|v| v.to_f64()).unwrap_or(0.0);
    let yes_ask = d.yes_best_ask.and_then(|v| v.to_f64()).unwrap_or(1.0);
    let no_bid = d.no_best_bid.and_then(|v| v.to_f64()).unwrap_or(0.0);
    let no_ask = d.no_best_ask.and_then(|v| v.to_f64()).unwrap_or(1.0);
    
    let yes_bid_depth = d.yes_bid_depth_usdc.to_f64().unwrap_or(0.0);
    let yes_ask_depth = d.yes_ask_depth_usdc.to_f64().unwrap_or(0.0);
    let no_bid_depth = d.no_bid_depth_usdc.to_f64().unwrap_or(0.0);
    let no_ask_depth = d.no_ask_depth_usdc.to_f64().unwrap_or(0.0);
    
    let spread = yes_ask - yes_bid;
    let mid = (yes_bid + yes_ask) / 2.0;

    let json = format!(
        r#"{{"timestamp":"{}","slug":"{}","token_id_yes":"{}","token_id_no":"{}","outcome":"YES","yes_bid":{},"yes_ask":{},"no_bid":{},"no_ask":{},"yes_bid_depth":{},"yes_ask_depth":{},"no_bid_depth":{},"no_ask_depth":{},"spread":{},"mid":{},"btc_price":{},"btc_return_5s":{},"btc_return_30s":{},"momentum":{},"seconds_to_expiry":{},"market_type":"btc-updown-5m","filled":false,"entry_price":null,"exit_price":null,"realized_pnl":0.0}}"#,
        now, slug, token_id_yes, token_id_no,
        yes_bid, yes_ask, no_bid, no_ask,
        yes_bid_depth, yes_ask_depth, no_bid_depth, no_ask_depth,
        spread, mid, btc_price, btc_return_5s, btc_return_30s, momentum, seconds_to_expiry
    );

    if let Ok(mut file) = std::fs::OpenOptions::new().create(true).append(true).open("training_ticks.jsonl") {
        use std::io::Write;
        let _ = writeln!(file, "{}", json);
    }
}